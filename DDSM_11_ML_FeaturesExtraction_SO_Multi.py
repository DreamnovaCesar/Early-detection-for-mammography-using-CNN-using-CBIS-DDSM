...
df = pd.read_csv("D:\DDSM\Calc Mammography\MultiDataCSV\DataFrame_Multi_DDSM_Data_ML_SO.csv")
path = "D:\DDSM\Calc Mammography\MultiDataCSV\DataFrame_Multi_DDSM_Data_ML_SO.csv"
...
DataFrameRAW, NameRAW = DataModelsExtract(MainKeys_Extract, ModelValuesRAW, TexturesFeatureGLCMImage)

DataFrameNO, NameNO = DataModelsExtract(MainKeys_Extract, ModelValuesNO, TexturesFeatureGLCMImage)

DataFrameCLAHE, NameCLAHE = DataModelsExtract(MainKeys_Extract, ModelValuesCLAHE, TexturesFeatureGLCMImage)

DataFrameHE, NameHE = DataModelsExtract(MainKeys_Extract, ModelValuesHE, TexturesFeatureGLCMImage)

DataFrameUM, NameUM = DataModelsExtract(MainKeys_Extract, ModelValuesUM, TexturesFeatureGLCMImage)

DataFrameCS, NameCS = DataModelsExtract(MainKeys_Extract, ModelValuesCS, TexturesFeatureGLCMImage)


AllModelValues(df, column_names, path, DataFrameRAW, MultiSVMConfigModels, MultiDataModelsML_SO, MultiDataCSV, NameRAW, 0)

AllModelValues(df, column_names, path, DataFrameNO, MLPConfigModels, MultiDataModelsML_SO, MultiDataCSV, NameNO, 6)

AllModelValues(df, column_names, path, DataFrameCLAHE, KNNConfigModels, MultiDataModelsML_SO, MultiDataCSV, NameCLAHE, 12)

AllModelValues(df, column_names, path, DataFrameHE, RFConfigModels, MultiDataModelsML_SO, MultiDataCSV, NameHE, 18)

AllModelValues(df, column_names, path, DataFrameUM, DTConfigModels, MultiDataModelsML_SO, MultiDataCSV, NameUM, 24)

AllModelValues(df, column_names, path, DataFrameCS, GBCConfigModels, MultiDataModelsML_SO, MultiDataCSV, NameCS, 30)
